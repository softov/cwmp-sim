...
function parseXmlDeclaration(buffer: any) {
...
function encodeEntities(string: string) {
  ...
  if (!string) return "";
  return string.replace(/[&"'<>]/g, (m: string) => entities[m]);
}
...
export default { parseXmlDeclaration, parseAttrs, decodeEntities, encodeEntities, parseXml };
